...
**Current version v1.20.0** (changelog in the collapsed block at the end of this section).
...
- **Port taken by your own Chrome? Lasso never touches it** (BUG-03 C): on `port_in_use_non_cdp`, a Lasso-owned dead instance self-heals automatically (`ledger_zombie_collected`); if the occupier is your manually-opened Chrome or another program, the error says so honestly — "user asset, will not kill, decide yourself or pick another port" — Lasso provides no kill escape hatch for non-ledger assets.
- For pure scraping without login state, `--mode headless` (BUG-03 B2): a headless instance does not occupy your Chrome's Dock slot (clicking the Chrome icon is never routed into the tool instance); the tradeoff is no window and `chrome-show` is ineffective — use hidden/visible for login flows.
