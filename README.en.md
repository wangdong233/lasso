...
**Current version v1.28.0** (changelog in the collapsed block at the end of this section).
...
<details>
<summary>📋 Changelog (v1.8 → v1.29.0 — click to see what each version changed)</summary>

- **v1.29.0** (unreleased pre-write · bug11: input-primitive completion + honest-signal batch — the "input guard layer" falsification re-analysis, doc/bugs/11) — the afternoon field report's P0 verdict ("tm.aliyun.com's input guard instantly clears programmatically set values") was **falsified** by a controlled white-box re-analysis: the real mechanism is the **state-integrity semantics of a React 16/17 controlled component** (script writes don't fire onChange → on blur/commit the framework rewrites the DOM back to the committed state — the value survives until blur, it does not vanish in milliseconds; the "six attacks all failed" was an observation-surface error: the submit went through `window.open` into a NEW tab, the original page's URL never changes → `wait url_contains` is structurally blind, while the uid-path fill completed a real search end-to-end that same day). Deliverables: (1) **`type` / `press` input-primitive completion** (the missing third of the click/fill/type triad): `action:"type"` + `selectors:{<uid|ref>:<text>}` — per-key dispatch through the CDP `Input.dispatchKeyEvent` **trusted input pipeline** (marked `isTrusted:true` by the browser, enters the framework input pipeline, React onChange fires normally, survives blur; same tier as Playwright's `page.keyboard.type()`; long texts are also per-key — covering the fault line where fill's uid path flips to an untrusted JS bulk-set path at ≥100 chars); `action:"press"` + `key:"Enter"` (modifiers split + failure-release guaranteed upstream) — implemented as mappings onto the locked upstream `type_text`/`press_key` tools (no direct CDP); submitKey is NOT exposed (submissions go explicitly through a follow-up press/click — a two-step chain is more auditable). (2) **`input_guard_suspected` honest signal** (advisory, eviction-sentinel paradigm): the ref fill/type path embeds three probes (non-native value setter installed / ~250ms in-page readback mismatch / React tracker divergence) — a hit attaches the signal + hint (switch to the trusted type pipeline, or ask the user to type physically); **signal only, NEVER auto-reroutes** (no automatic primitive switch/retry/channel change), probes are pure reads with zero side effects (synthesizing blur for probing is forbidden). (3) **wait-timeout observation-surface teaching**: `wait_timeout` errors gain a window.open new-tab teaching sentence (the original page's URL never changes — this timeout shape now carries its attribution clue) + TROUBLESHOOTING §2.19 (including the `navigate → wait{selector} → fill → press/click` chain recipe; the fill-timing-race hypothesis was falsified with no reproducer — no defensive code written). (4) **no_reload semantics clarification**: the describe gains the "this is NOT a skip-navigation flag" clause (substantially different URLs always navigate; identical URLs never navigate at all — that's url rule 2, unrelated to this flag) + §2.20 four-row behavior matrix + a semantics-pinning spec (description and behavior now pinned together by tests). (5) **governance/13 synthetic-input ethics boundary declaration**: the sanctioned face = standard primitives + honest signals + the trust-ladder doc; **Rejected-by-design** = keystroke-rhythm randomization/human-mimicry delays, `_valueTracker` spoofing, accessor un-hooking, isTrusted forgery, and every wrapper that "makes fill look like human typing" (deceiving site-side mechanisms ≠ input capability; only an explicit, item-by-item user ruling can overturn it). Stance: **the wall is there and we tell you** — only two legal ways through (switch to the trusted primitive, or ask the user to type physically); lasso never decides to attack the wall for you.
- **v1.29.0** (unpublished pre-write · bugs/11 U-T half: type/press keyboard primitives + honest input-guard signal, doc/bugs/11) — ① **`type` action** (completing the click/fill/type input-primitive triad): `selectors = { uid|ref: text }` types through the upstream `type_text` per-key CDP dispatch — **the trusted input pipeline** (`isTrusted:true`; framework onChange fires normally — use it when script-set values get rewritten on blur/commit, or when fill values ≥100 chars fall into the script-set path); **appends** (= Playwright `page.type`; fill replaces); ref-path focus-receipt failure reports `type_focus_failed` (didnt — no guessing). ② **`press` action**: `options.key` (e.g. 'Enter' / 'Control+A' — modifiers parsed + released-on-failure upstream); submissions stay explicit two-step chains (no submitKey, no auto-submit). ③ **`input_guard_suspected` signal** (advisory; outcome stays worked): after ref-path fill/type one read-only three-probe evaluate (non-native value setter / filled value not kept on delayed readback / React `_valueTracker` divergence — the tm.aliyun.com mechanism made visible); on hit the response carries `data.input_guard_suspected {checks,target,at_ms}` + a hint with three legal exits (switch to type / re-extract for the snapshot-uid path / ask the user to type physically) — **signal only, never auto-rerouting** (eviction-sentinel red line: probes are pure reads + in-page sleep; synthesizing blur for probing is forbidden). ④ HighRiskGate gains the `type` branch: RTE-blacklisted elements block type exactly like fill (closing the rename-bypass gap; press has no DOM target and stays evaluate-tier unblocked, documented). ⑤ Upstream-contract dead list gains `type_text`/`press_key` (P10 pre-gate false-red fix) + explicit description budget-cap re-baselining (fleet 59,850 / HEADLESS 13,900 / L2 2,300 — §5.1-3 single-topic commit + equal-mass dedup trims). Ethical red line (heaviest of this batch): type/press are standard input primitives on par with Playwright `page.keyboard.type()`; implementing typing-rhythm mimicry, `_valueTracker` spoofing, accessor un-hijacking, isTrusted forgery or any other wall-penetration trick is **rejected by design** — the wall is there, we tell you, crossing it stays the caller's call. Local input-guard fixture regression (opt-in `LASSO_TEST_INPUT_GUARD=1`, bound to 127.0.0.1): all three mechanism families check-typed + type/uid paths verified to survive blur.
