...
- **v1.24.0（未发版，本批已合入工作树）**：🔴 **行为变更（BUG-06，12h 幽灵常驻事故根治，doc/bugs/06）**——① `--idle-ms 0` / `LASSO_LAUNCH_IDLE_MS=0` 语义从「永不回收」改为「**无活动 24 小时硬顶回收**」（自最近活动起算——lasso browse 自动落盘 touch 文件或外部 `touch ~/.cache/lasso/chrome-touch-<端口>` 续命，**在用永不触发**；`LASSO_LAUNCH_HARD_CAP_MS` 新键可调/禁）；真·无限常驻 = `--idle-ms 0 --no-hard-cap` 双旗，或部署级 `LASSO_LAUNCH_HARD_CAP_MS=0`。visible/用户认领（chrome-show）档豁免不变（r2：收割判定与执行间隙落下的认领同样豁免——reaper 默认收割出口在 kill 时刻重估 userTakenAt，对抗复审 P2 TOCTOU 闭口）；渲染档零变化（`LASSO_RENDER_IDLE_MS` 独立，INV-94 tripwire 钉死）。**升级瞬间存量超龄 idle-0 记录将被首个收割 tick（≤15s）回收**——属预期（该类实例即事故幽灵形态）；依赖 0=无限且不 touch 的 nohup 配方请迁移（TROUBLESHOOTING §9.3 / KEY-GUIDE）。② 跨进程活动真源（A-7）：`lasso browse` 活动现自动落盘 touch 文件——执守收割宿主可见（此前只写 server 进程内存，双宿主并时有 24h/30min 误杀盲区，同型同修）；③ 超龄可观测（B）：chrome-status/doctor 对硬顶过半（>50%）的在案实例持续给出 warn 咨询块（预期回收时刻 + touch 续命法 + 用户 chrome-stop 出口），零新增 kill/指令面；④ 全局 `LASSO_LAUNCH_IDLE_MS=0` 下 reaper 以 cap-only 模式运行（D2——不再连硬顶兜底一起拆）。对抗复审 r1 两项 P1 采纳（N1 跨进程盲区 / N2 缺省反转 0 下沉装配层）；r2 闭 reaper TOCTOU（P2——默认收割出口 kill 时刻对 userTakenAt 重估）；新不变量 INV-94。
