...
<details>
<summary>📋 更新日志（v1.8 → v1.29.0，点开看每版改了什么）</summary>

- **v1.29.0**（未发布预写 · bug11：输入原语补齐 + 诚实信号批——「输入保护层」证伪重析，doc/bugs/11）——下午实机报告判定的 P0「tm.aliyun.com 输入保护层瞬间清除程序化设值」经白盒受控重析**证伪**：真机理是 React 16/17 受控组件的**状态完整性语义**（脚本写入不触发 onChange → blur/commit 时框架把 DOM 回写为已提交状态——值存活到 blur，非「毫秒级消失」；「六路全败」实为观察面错误：submit 走 `window.open` 开新 tab、原页 URL 永不变 → `wait url_contains` 结构性失明，而 uid 路 fill 当天全链路完成真实搜索）。本批交付物：① **`type` / `press` 输入原语补齐**（click/fill/type 三原语的第三缺）：`action:"type"` + `selectors:{<uid|ref>:<text>}`——逐字派发走 CDP `Input.dispatchKeyEvent` **可信输入管道**（浏览器标记 `isTrusted:true`，进框架输入管道、React onChange 正常触发、blur 后存活；与 Playwright `page.keyboard.type()` 同级；长文本亦逐字——覆盖 fill uid 路 ≥100 字符翻转进 JS 批量设值不可信管道的断层）；`action:"press"` + `key:"Enter"`（修饰键由上游拆解 + 失败释放保证）——实现为对已锁定上游 `type_text`/`press_key` 的映射（禁 CDP 直连）；不暴露 submitKey（提交显式走后续 press/click，两步链更可审计）。② **`input_guard_suspected` 诚实信号**（advisory，驱逐哨兵同范式）：ref 路填充/键入内嵌三探针（value setter 非原生安装态 / ~250ms 页内读回不一致 / React tracker 分叉）——命中即附信号 + hint（换 type 可信管道，或请用户物理输入）；**只供信号绝不自动改道**（不自动换原语/重试/通道），探测纯读零副作用（禁为探测合成 blur）。③ **wait 超时观察面教学**：`wait_timeout` 错误追加 window.open 新 tab 教学句（原选中页 URL 永不变——该形态超时首次自带归因线索）+ TROUBLESHOOTING §2.19 新 tab 观察面（含 `navigate → wait{selector} → fill → press/click` 链配方；fill 时序竞争假说无 reproducer 证伪登记，不写防御代码）。④ **no_reload 语义澄清**：describe 补「不是跳过导航旗标」句（实质不同 URL 恒导航；同 URL 恒零导航是 url 规则 2、与本键无关）+ §2.20 行为矩阵四行 + 语义钉 spec（描述与行为从此被测试钉死为一体）。⑤ **governance/13 合成输入伦理边界声明**：正面面 = 标准原语 + 诚实信号 + 信任阶梯文档；**Rejected-by-design** = 键入节奏随机化/人类拟态延迟、`_valueTracker` 欺骗、accessor 反劫持、isTrusted 伪造、「让 fill 看起来像人输入」的一切包装（欺骗站点侧机制 ≠ 输入能力；仅用户显式逐条裁决可推翻）。立场：**墙在那里我们告诉你**——穿墙只有两种合法形态（换可信原语 / 请用户物理输入），绝不替用户决定攻墙。
- **v1.29.0**（未发布预写 · bugs/11 U-T 半：type/press 键盘原语 + 输入保护层诚实信号，doc/bugs/11）——① **`type` action**（click/fill/type 三原语补齐第三缺）：`selectors = { uid|ref: text }` 经上游 `type_text` 逐字派发 CDP 键盘事件——**可信输入管道**（`isTrusted:true`，React/lowcode 框架 onChange 正常触发；脚本设值被 blur/commit 回写吞掉、或 fill ≥100 字符落入脚本设值路径的场景用它）；**追加语义**（=Playwright `page.type`，fill 才是替换）；ref 路 focus 回执失败显式报 `type_focus_failed`（didnt，不猜）；② **`press` action**：`options.key`（如 'Enter' / 'Control+A'——修饰键由上游拆解 + 失败释放保证）；提交保持显式两步链（type 不带 submitKey、不自动提交）；③ **`input_guard_suspected` 信号**（advisory，outcome 照旧 worked）：ref 路 fill/type 后一次纯读三探针（value setter 非原生 / 填充值延时读回丢失 / React `_valueTracker` 分叉——tm.aliyun.com 实测机理的显式化），命中时响应附 `data.input_guard_suspected {checks,target,at_ms}` + hint 三条合法出路（换 type 可信管道 / 重 extract 走快照 uid 路 / 请用户物理输入）——**只供信号绝不自动改道**（驱逐哨兵同款红线：探针纯读 + 页内 sleep，禁为探测合成 blur）；④ HighRiskGate 补 `type` 分支：RTE 黑名单元素上 type 与 fill 同拦（封「换 action 名绕过 identity 通道守卫」的口径裂缝；press 无 DOM 目标与 evaluate 同档不拦，注释文档化）；⑤ 上游契约死列表补 `type_text`/`press_key`（P10 前置门自动覆盖假红修复）+ 描述预算帽显式提帽（fleet 59,850 / HEADLESS 13,900 / L2 2,300——§5.1-3 独立单主题 commit + evaluate 块 URL 语义重复段等量去重）。伦理红线（本批最重）：type/press 是与 Playwright `page.keyboard.type()` 同级的**标准输入原语**；**拒绝实现**键入节奏随机化/人类拟态延迟、`_valueTracker` 欺骗、accessor 反劫持、isTrusted 伪造等一切穿透手法（rejected-by-design——墙在那里我们告诉你，穿不穿交给调用方）。本地保护层 fixture 回归（opt-in `LASSO_TEST_INPUT_GUARD=1`，绑 127.0.0.1）：三型机制（受控回写/accessor 劫持/异步重置）check 型对号 + type/uid 路 blur 后存活全实证。
