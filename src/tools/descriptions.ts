...
export const BROWSE_HEADLESS_DESCRIPTION = [
  ...
  "  screenshot — PNG to disk; preview holds the path. options.screenshot.filePath",
  "               sets the output path (default /tmp/lasso-screenshot-<uuid>.png);",
  "               large screenshots (≥2MB) go via the upstream filePath direct-write",
  "               path with automatic image-block fallback (both verified PNG magic)",
  ...
  "  evaluate   — eval JS (opts.js). BOTH forms accepted (dual-form): a function",
  "               expression like `() => document.title` / `async () => {...}` is",
  "               passed through as-is (upstream auto-invokes); a statement body",
  "               like `return document.title` is wrapped and invoked. Either works",
  "",
