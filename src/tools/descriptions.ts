...
export const BROWSE_HEADLESS_DESCRIPTION = [
  ...
  "  navigate   — go to url. HASH-ONLY NAVIGATION (BUG-08 D-1): when the target",
  "               differs from the current page ONLY by #hash, SPAs treat it as",
  "               same-document (no reload — the page keeps showing OLD query",
  "               results while the URL says new: silent stale data). lasso detects",
  "               this and reloads on the target hash by default, echoing",
  "               data.same_document_navigated:true + same_document_reloaded:true.",
  "               Opt out with options.no_reload:true (app handles its own hash",
  "               routing; stale-data risk is then yours) — still annotated, e.g.",
  '               browse_headless({url:"https://app.example.com/#/report", action:"snapshot", options:{no_reload:true}})',
  ...
  "               SCOPE (BUG-05 A): options.screenshot is read ONLY by this action",
  "               (it navigates first — one-step navigate+shoot IS action=screenshot);",
  "               passing it to navigate etc. is ignored and surfaced in",
  "               data.ignored_options — never silently dropped",
  "  extract    — full-page text extraction",
  "  click      — click an a11y uid (opts.selectors.click = uid)",
  "  fill       — fill_form (opts.selectors = { uid: value, ... })",
  "  type       — per-key typing, the trusted CDP input pipeline (opts.selectors =",
  "               { uid: text, ... }, any length; ref keys too; appends to existing",
  "               text — fill replaces). Trusted keys reach framework onChange where",
  "               script-set values get rewritten on blur — use for long texts (fill",
  "               types <100-char values per key, longer are script-set) and when a",
  "               fill seems silently dropped; NOT desktop's kind:'type' (AX replace)",
  "  press      — press a key/combo (opts.key, e.g. 'Enter' / 'Control+A'); modifiers",
  "               parsed + released upstream; submissions stay explicit (chain a",
  "               press/click step — type never auto-submits)",
  "  wait       — wait for a condition (opts.expect: text / selector / url_contains —",
  "               at least one required, same contract as step postconditions).",
  "               text-only waits use the native upstream wait_for; selector/",
  "               url_contains (and gone:true absence waits) poll via evaluate",
  "               every 100ms (default 5s, expect.timeout_ms overrides). url omitted",
  "               = current-page wait (BUG-08 D-3: waits on the page this channel",
  "               already manages — requires an active session, same contract as",
  "               current-page screenshot; no session → didnt + no_active_session:",
  "               current_page_wait, never silently waits on a blank page). NOTE:",
  "               passing url to a wait NEVER navigates (did_navigate stays false)",
  "  evaluate   — eval JS (opts.js). THREE forms accepted: a function expression like",
  "               `() => document.title` / `async () => {...}` is passed through as-is",
  "               (upstream auto-invokes); an IIFE like `(async () => {...})()` is",
  "               wrapped as an expression body and its RESULT is returned; a statement",
  "               body like `return document.title` is wrapped and invoked. All work.",
  "               RETURNS (bug10 B): any other single-line EXPRESSION (`JSON.stringify({...})`,",
  "               `({a:1})`, `document.title`) is auto-wrapped and its VALUE returned;",
  "               multi-statement bodies (declarations / `;` / multiple lines) return",
  "               undefined unless you `return` — prefer one expression per call; an",
  "               undefined statement-body result echoes data.js_form + hint.",
  "               URL SEMANTICS (bug09 decision B, unified for ALL url-aware",
  "               actions, single-action calls): url ≠ current page → lasso navigates",
  "               FIRST, then runs the JS there (did_navigate:true); url == current",
  "               page runs in place (did_navigate:false); omit url = current-page JS",
  "               (same-session contract). Full rules: UNIFIED URL SEMANTICS below.",
  "               TIMEOUT (BUG-08 A-2): options.budget_ms sets THIS call's MCP timeout",
  "               (default 120s, env LASSO_EVAL_TIMEOUT_MS; zod cap 600s) — pass e.g.",
  "               budget_ms:300000 for long batch JS (20+ in-page requests + sleeps).",
  "               A timeout returns typed error 'mcp_request_timeout:' + hint — the",
  "               browser is NOT dead; the upstream mutex wedge self-heals in ~3min",
  "  console    — read THIS page's console messages collected since the last",
  "               navigation (current-session semantics: a bare console call does NOT",
  "               re-navigate — navigate first in your flow). options.console_level",
  "               is a severity threshold ('error' ⊂ 'warn' ⊂ 'info' ⊂ 'debug' — each",
  "               level includes the more severe ones); options.console_limit=N keeps",
  "               the MOST RECENT N messages after filtering",
  "  network    — coarse per-page resource list since the last navigation",
  "               (options.network_filter: xhr/fetch/img/3rd-party/all); the",
  "               standalone network tool remains the full featured surface",
  "",
  ...
  "",
  "CURRENT-PAGE SCREENSHOT (BUG-07) — omit url with action=screenshot to shoot the",
  "  page this channel is ALREADY managing, with zero navigation: freeze a CSS/WAAPI",
  "  animation via evaluate (document.getAnimations().forEach(a=>a.pause())), then",
  "  {action:'screenshot'} (no url) captures the frozen mid-frame for pixel-level",
  "  forensics; follow with {action:'console'} — same session, single tool. Success",
  "  returns data.url='current-page' (a literal, not a parsed URL — resolving the",
  "  real page URL is out of scope). No active session (cold channel / page closed /",
  "  rotated by self-heal) → outcome=didnt + error='no_active_session:",
  "  current_page_screenshot' + a rebuild hint — it never silently shoots a blank",
  "  page, and no cross-channel fallback fires for current-page requests.",
  "",
  "FRESH PROFILE ESCAPE HATCH (BUG-08 C) — options.freshProfile:true recycles this",
  "  channel's whole identity BEFORE the call runs: new throwaway profile dir +",
  "  deterministic stealth rotation + full stack respawn. Use it when the site has",
  "  server-side-fingerprinted this headless instance (symptom: queries silently",
  "  return zero results with no captcha, no error). The new identity serves all",
  "  subsequent calls until idle reap / the next freshProfile / server exit — NOT a",
  "  per-call rotation (in-session identity churn is itself a bot signal). Success",
  "  echoes data.fresh_profile:true. browse_logged_in rejects it (your real Chrome)",
  "  with error fresh_profile_not_supported_on_logged_in.",
  "",
  "UNIFIED URL SEMANTICS (bug09 decision B) — one rule for every url-aware action",
  "  (evaluate / snapshot / extract / screenshot / network / pdf), single-action",
  "  calls:",
  "   - url ≠ current page → lasso navigates FIRST, then runs the action there",
  "     (echoes did_navigate:true — extract can no longer silently read whatever",
  "     residual page the session holds while claiming your url);",
  "   - url == current page → runs in place, ZERO navigation (did_navigate:false;",
  "     same-url reloads would destroy SPA state — screenshot/network/pdf on the",
  "     current page no longer reload it);",
  "   - url omitted (screenshot / wait / evaluate only) → current-page mode,",
  "     requires an active session on this channel.",
  "  navigate is navigation itself (did_navigate always true); wait/click/fill",
  "  never navigate (their url is decorative — did_navigate stays false).",
  "  steps chains: ensure-nav does NOT apply per step — the chain runs on the",
  "  page established beforehand (action:navigate + steps, or a leading",
  "  navigate step); a chain's url is the resource key, not a per-step target.",
  "  Chain-level echo IS provided (bug09 §8.B): data.did_navigate mirrors the",
  "  chain's navigation facts (leading action:navigate or a worked navigate",
  "  step ⇒ true; residual-page / same-url chains ⇒ false), and data.final_url",
  "  is the tail-read truth of where the chain actually ran — a chain that ran",
  "  on the residual page reports that page, never the requested url echoed back.",
  "",
  "Use for:",
  ...
  '  browse_headless({url:"https://example.com", options:{steps:[{action:"click",selectors:{click:"@uid"}},{action:"wait",expect:{text:"Saved",timeout_ms:5000}}]}})',
  '  browse_headless({url:"https://example.com", action:"snapshot", options:{freshProfile:true}})',
  '  browse_headless({url:"https://example.com", action:"type", options:{selectors:{"@uid":"per-key long text"}}})',
  '  browse_headless({action:"press", options:{key:"Enter"}})',
  "",
  "[Prefer browse_logged_in]: auth-needed sites. [Prefer search]: keyword discovery.",
  "",
  "SECURITY: URL is SSRF-checked (private IPs denied; extend via LASSO_SSRF_ALLOW_RANGES).",
  "",
  "Args:  url (str, optional for action=screenshot / wait / evaluate = current-page mode;",
  "                               required otherwise) — http(s) URL (userinfo @ forbidden);",
  "                               file:// URLs are rejected by default (opt-in per",
  "                               dir via env LASSO_ALLOW_FILE_FROM, colon-separated).",
  '                               When allowed: browse_headless({url:"file:///Users/me/report.html", action:"extract"})',
  "                               Omit ONLY with action=screenshot (shoot),",
  "                               action=wait (await condition) or action=evaluate",
  "                               (run JS) to act on the page this channel already",
  "                               manages (zero navigation); any other action",
  "                               without url → didnt +",
  "                               'url_required_for_action:<action>'",
  "       action (str, default 'snapshot')",
  "       options (object, optional) — { selectors, js, expect, screenshot, ... };",
  "       most option keys are action-scoped (screenshot → action=screenshot,",
  "       extract_mode/include_refs → action=extract, js → action=evaluate, ...).",
  "       Keys you pass that the chosen action does NOT consume are echoed back in",
  ...
  "         echoes data.did_navigate (true/false) in single-action calls — e.g.",
  '         browse_headless({url:"https://example.com", action:"extract"}) →',
  "         did_navigate:false if already on the page (zero reload); see",
  "         UNIFIED URL SEMANTICS above; steps chains echo chain-level",
  "         did_navigate (leading navigate ∨ worked navigate step) and",
  "         final_url = tail-read truth of the page the chain actually ran on",
  "         (a leading navigate's final_url seeds it only when the tail read fails).",
  "         A response may also carry data.eviction_suspected {from,to,at_ms}",
  "         + hint when the channel observes the page drifting to a different",
  "         host after lasso's own navigation settled (suspected site eviction",
  "         of automation — NOT confirmed; an earlier click's side effect is",
  "         indistinguishable at this layer). Advisory, one-shot (attached to a",
  "         single response): L1 atomic reads (extract/snapshot with url) may",
  "         still work; for JS residency ASK THE USER FIRST, then retry with",
  "         browse_headed (opens a real on-screen window).",
  "         NOTE (bug10 D): form submissions may be silently swallowed by anti-bot",
  "         layers — click worked means 'event delivered', NOT 'accepted'; verify",
  "         with wait url_contains/text or extract — never trust click worked alone",
  "         (doc/usage/02-TROUBLESHOOTING.md §2.18).",
  "         A ref-path fill/typing probe may attach data.input_guard_suspected",
  "         {checks,target,at_ms} + hint (advisory; outcome stays worked) when the",
  "         site appears to rewrite script-set values (non-native value setter /",
  "         value not kept after fill / React tracker divergence).",
  ].join("\n");
...
export const BROWSE_LOGGED_IN_DESCRIPTION = [
  ...
  "",
  "IF CHROME DIED AFTER LAUNCH (idle reaper, doc/bugs/02 + BUG-03 A1): a launched",
  "Chrome with no activity is auto-reaped after the idle window. CLI launches",
  "default to --idle-ms 1800000 (30min) since BUG-03 A1; external CDP consumers",
  "keep it alive while in use by touching",
  "`~/.cache/lasso/chrome-touch-<port>` (mtime = activity signal); explicit",
  "`--idle-ms 0` = never auto-reap (long-session opt-out).",
  "",
  "IF LAUNCH FAILS WITH port_in_use_non_cdp (BUG-03 C): lasso NEVER kills user",
  "assets — if the port is held by your own Chrome, lasso will not kill it;",
  "report to the user to decide (close it manually) or launch with a different",
  "--port. Lasso-owned zombies on that port are self-healed automatically",
  "(ledger_zombie_collected event) — EXCEPT zombies the user has taken over",
  "(user_taken_asset: claimed via user activation/explicit chrome-show, or a",
  "visible login window): never auto-killed (fold via chrome-hide); only the user closing",
  "them or the user running chrome-stop themselves is a valid exit — agents",
  "must not run chrome-stop against a user_taken_asset on their own.",
  "",
  "DOES NOT:",
  ...
  "network / click / ...), including extract_mode and include_refs interactive",
  "refs (v1.17), console_level/console_limit (BUG-05 C), and current-page screenshot (BUG-07:",
  "  omit url + action=screenshot shoots this channel's managed page with zero",
  "  navigation — evaluate-freeze → shoot-current-frame → console forensics all",
  "  in one session. Requires THIS channel to have navigated first; no active",
  "  session → didnt + no_active_session:current_page_screenshot — shooting",
  "  whatever tab your Chrome happens to have selected is a separate feature,",
  "  deliberately not built). BUG-08 adds: current-page wait (omit url +",
  "  action=wait — same session contract), hash-navigation default reload (D-1),",
  "  evaluate ensure-navigation (D-2), and automatic port discovery when default",
  "  9222 has no CDP (E-1 — result retrieval_method carries",
  "  +auto_discovered_port:<port>). bug09 decision B unifies url semantics for",
  "  every url-aware action (evaluate/snapshot/extract/screenshot/network/pdf;",
  "  single-action calls — steps chains run on the page established by",
  "  action:navigate or a leading navigate step, and echo chain-level",
  "  did_navigate + tail-read final_url as qualified in browse_headless):",
  "  url ≠ current page navigates first (did_navigate:true); url == current page",
  "  runs in place with zero reload (did_navigate:false — SPA state preserved);",
  "  url omitted is now valid for evaluate too (current-page JS, same session",
  "  contract). freshProfile is",
  "  headless-only and rejected here (your real Chrome identity is never rotated).",
  "  High-risk",
  ...
  "",
  "EXAMPLES (same schema + call shape as the browse family):",
  '  browse_headless({url:"https://example.com", action:"evaluate", options:{js:"() => document.title"}})',
  "",
  ...
  "default allowlist so the CDP connect itself is permitted. file:// URLs are",
  "rejected by default; same LASSO_ALLOW_FILE_FROM opt-in applies.",
  "",
  "Args:  url (str, optional for action=screenshot / wait / evaluate = current-page",
  ...
  "                               chains echo chain-level did_navigate +",
  "                               tail-read final_url, as qualified there).",
  "       action (str, default 'snapshot')",
  "       options (object, optional)",
  "",
  "Returns: InteractResult<BrowseResult> as JSON text (url semantics shared with",
  '  browse_headless — e.g. browse_headless({url:"https://example.com", action:"extract"})',
  "  echoes data.did_navigate:true/false; already on the page ⇒ false, zero reload).",
].join("\n");
...
export const BROWSE_HEADED_DESCRIPTION = [
  ...
  "",
  "Eviction companion: when a browse response carries data.eviction_suspected",
  "  (a cross-host drift the channel observed after its own navigation — a",
  "  site evicting automation is one suspected cause, an earlier click's side",
  "  effect another; NOT confirmed), ASK THE USER FIRST, then retry here for",
  "  JS-resident needs. If you instead see evaluate die with",
  "  'Execution context was destroyed' — same eviction form",
  "  (signal unavailable when the redirect stays on-host);",
  "  ASK THE USER FIRST, then retry here.",
  "",
  "  NOTE (bug10 decision D): on sites whose submission layer challenges (e.g. an",
  "  Aliyun slider captcha after a search submit), the captcha iframe is",
  "  cross-origin — evaluate and AX are both blind to it; see the desktop tool",
  "  description for the combined act + screenshot-diff recipe",
  "  (doc/usage/02-TROUBLESHOOTING.md §2.18).",
  "",
  "EXAMPLES:",
...
export const DOCTOR_DESCRIPTION = [
  ...
  "  zhipu_keys_retired (v1.17) / machine_search_mcp /",
  "  download_engines & chrome_ledger_inventory (v1.30+: engines; Chrome ledger; visible strays → chrome-hide).",
  "",
...
export const DESKTOP_DESCRIPTION = [
  ...
  "               RECEIPTS (bugs/10): coordinate mouse actions echo cursor_after",
  "               {x,y} + landed per action (read back after the final post;",
  "               ok:false + cgevent_no_landing = the event did NOT land).",
  "               Dispatch-level physical_input.attribution",
  "               = \"physical\" means a physical mouse event raced your synthetic",
  "               one (resend or verify with expect/landing receipt — the action",
  "               may have been classified as a drag). expect is the receipt for",
  "               AX-visible targets. For AX-BLIND targets (cross-origin captcha",
  "               iframes: evaluate AND find both cannot see inside) the combo",
  "               recipe is: act → screenshot (screenshot_region before/after)",
  "               → YOU compare the two images.",
  "  wait       — poll for window/element/appFrontmost (tri-state)",
  "  screenshot — PNG to disk (default /tmp/lasso-screenshot-<uuid>.png); preview",
  "               holds the path. options.inline_base64:",
  "               true returns screenshot_base64 inline (full-screen",
  "               ≈860KB). Fallback for pictureOnly nodes (canvas/Metal).",
  "  doctor     — TCC / AX read-rate / signature / Rust helper health + #22",
  "               cgevent_delivery_selftest (opt-in LASSO_DOCTOR_INPUT_SELFTEST=1",
  "               — moves your cursor 7px and restores it; off by default).",
  "",
...
export const FETCH_URL_DESCRIPTION = [
  ...
  "",
  "FAILURE GRANULARITY (BUG-08 E-2): network-layer errors classify the undici",
  "  cause code — error is 'fetch_failed:<kind>:<detail>' with kind ∈",
  "  dns_failed (ENOTFOUND/EAI_AGAIN — local DNS/proxy env) / connect_refused /",
  "  connect_timeout / tls_failed (CERT_*/ERR_TLS_*) / aborted_timeout (your",
  "  options.timeout_ms hit) / other. Use it to tell \"my proxy/DNS is broken\"",
  "  from \"the site refused us\" (bot-walls come back as 2xx bodies, not errors).",
  "",
...
export const SCREENSHOT_DESCRIPTION = [
  ...
  "",
  "CURRENT-PAGE MODE (BUG-07): omit url to shoot the CURRENT frame of the page this",
  "  tool's headless session is already managing (same managed subprocess as",
  "  browse_headless — 'current page' is the same page in both entries). Pair with",
  "  browse_headless evaluate: freeze an animation mid-frame, then screenshot",
  "  without url — no re-navigation, the frozen state survives. No active session",
  "  → outcome=didnt + error='no_active_session:current_page_screenshot' + a",
  "  rebuild hint (never silently shoots a blank page). data.url echoes",
  "  'current-page' (literal). For a logged_in session's current frame use",
  "  browse_logged_in {action:'screenshot'} without url instead.",
  "",
  "EXAMPLES:",
  '  screenshot({url:"https://example.com", options:{full_page:true}})',
  '  screenshot({})',
  "",
  "Use for:",
  ...
  "",
  "Args:  url (str, optional)          — http(s) URL (userinfo @ forbidden); omit",
  "                                      for current-page mode (shoot this tool's",
  "                                      managed headless session's current frame,",
  "                                      zero navigation — see above)",
...
export const ADMIN_DESCRIPTION = [
  ...
  "  - tab_restore         : {reason} restore the user's original tab list (mutation)",
  "  - chrome_status       : {port?} classify a CDP port's occupier (READ-ONLY: ledger",
  "                         zombie / user-owned / lasso orphan / user asset / external /",
  "                         probe_failed) with evidence + user_paste_pack. Answers who",
  "                         owns this port and what am I allowed to do — agents NEVER",
  "                         get a kill command (never_kill_user_asset); the only allowed",
  "                         chrome-stop form is the gated 'chrome-stop --zombie-gate",
  "                         --port N' (ledger zombie branch only)",
  "  - browser_recycle     : {channel:'headless', freshProfile?, reason} controlled",
  "                         in-process restart of the INTERNAL MCP stack (BUG-08 B-1:",
  "                         the sanctioned exit for a wedged/polluted headless stack —",
  "                         procs map + closed flag + session guards reset atomically;",
  "                         freshProfile:true goes further = full new identity, BUG-08 C).",
  "                         Detached Chrome is chrome-stop's domain and is NEVER touched",
  "",
  ...
  "  - \"recycle/restart the headless stack\" or \"fresh identity for browse_headless\"",
  "    (browser_recycle; anti-bot lockout escape hatch — BUG-08)",
  "",
  "EXAMPLES:",
  '  admin({action:"browser_recycle", channel:"headless", reason:"wedged stack"})',
  "",
  "DO NOT USE FOR:",
