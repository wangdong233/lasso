/**
 * steel tool 注册（parse14 §3.4 —— v1.6 Steel 自托管 cloud 浏览器工具）
 *
 * 单一 cloud 浏览器工具，schema 与 browse_headless / browse_logged_in / browserbase 同构（同 action 集）。
 * 唯一差异：路由到 SteelChannel（extends BrowseChannel，复用 actionDispatch Map）。
 *
 * 装配铁律（parse14 §3.3 + INV-25 + INV-74）：
 *  - 本 register 仅在 index.ts **双重解锁**（LASSO_ALLOW_CLOUD_BROWSER=true + STEEL_ENDPOINT）
 *    通过后才被调用；未解锁时 server.listTools() 完全不含 steel（默认 OFF 零回归）
 *  - FallbackDecider 前置 PolicyGate.check 已在 index.ts 注入；本 tool 的 plan 仅含
 *    browse_cloud_steel（terminal channel，cloud 浏览器不做下一跳 fallback ——
 *    parse5 §3.2.1 「cloud 是 fallback 链尾」语义）
 *
 * 边界（parse14 §1.3 + parse5 §3.2.1）：
 *  - 不解 2FA / CAPTCHA（cloud_browser_requires_manual_switch 升级信号由 StealthEngine 给）
 *  - 不与 browse_logged_in 共享登录态（cloud Chrome 是独立 session）
 *  - 不在 forest 调度层注册（model 显式调 steel tool，不经 interact_*）
 *
 * 借鉴：
 *  - browserbase.ts registerBrowserbaseTool（schema / ssrfGuard / InteractResult 包装范式）
 *  - parse14 §3.1 SteelChannel（lazy connect + mutex + stealth inject hook）
...
import type { SteelChannel } from "../channels/SteelChannel.js";
...
import { STEEL_DESCRIPTION } from "./descriptions.js";
import { steelAnnotations } from "./annotations.js";

// ============================================================
// Schema（与 browserbase.ts browserbaseSchema 同构 —— 同 action 集）
// ============================================================
const steelSchema = {
  url: z.string().url(),
 ...
 * @param steel   SteelChannel（自托管 cloud Chrome，extends BrowseChannel）
 * @param decider 单一 fallback 引擎（已注入 PolicyGate；cloud_blocked 走 policy_blocked）
 * @param ssrfConfig  SSRF allowRanges / denyRanges（同 browse_headless / browse_logged_in / browserbase）
 *
 * INV-1（browse 唯一入口）：本 tool 不算「browse 入口」（grep INV-1 只匹配 browse_headless /
 *   browse_logged_in）；cloud 浏览器是平行工具，不复用 browse 名空间。
 * INV-23（fallback 链不跨 surface）：本 tool 的 plan 仅含 browse_cloud_steel（terminal）；
 *   cloud 失败由 PolicyGate / retrieval_method=cloudflare_manual_switch 显式升级，不跨 surface。
 * INV-74（v1.6）：SteelChannel extends BrowseChannel（平级兄弟子类，禁嵌套 / 禁自造 fallback）。
 */
export function registerSteelTool(
  ...
  server.tool(
    ...
    async (args) => {
      ...
      // SSRF 守门：与 browse_headless / browse_logged_in / browserbase 同（cloud Chrome 也禁私网）
      ...
      // cloud 通道失败信号（cloudflare_manual_switch / steel_no_endpoint）由 model 显式处理。
      const plan = {
      ...
      const result = await decider.runWithFallback(plan, async (name) => {
        if (name === "browse_cloud_steel") {
          return steel.browse(url, action, options);
        }
