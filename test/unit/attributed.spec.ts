 ...
 *  - 单源 served_by 透传（"search.machine_mcp"）
 *  - fanout 合并 served_by（"search.machine_mcp,search.brave"）
...
describe("withAttribution — 基础行为", () => {
  it("每条结果获得 served_by + original_rank", () => {
    ...
    const attributed = withAttribution(r, "search.machine_mcp");
    expect(attributed).toHaveLength(3);
    expect(attributed[0].served_by).toBe("search.machine_mcp");
  ...
  it("fanout 合并 served_by：每条都带 'search.machine_mcp,search.brave'", () => {
    const r = makeResult([
      { title: "T1", url: "https://1.test" },
      { title: "T2", url: "https://2.test" },
    ]);
    const attributed = withAttribution(r, "search.machine_mcp,search.brave");
    expect(attributed.every((a) => a.served_by === "search.machine_mcp,search.brave")).toBe(true);
  });
  ...
  it("纯函数：不修改原 SearchResult.results", () => {
    ...
    withAttribution(r, "search.machine_mcp");
