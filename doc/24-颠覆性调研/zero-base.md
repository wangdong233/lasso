...
> 方法论纪律（doc/23）：①问题从零基视角出发（用户要什么/约束是什么），**本文 §1-§4 写作时未读任何 Lasso 源码**，仅基于四份姊妹调研（scan-agent-api / scan-edge / scan-mcp-proto / scan-model-native）+ 本轮新增外部证据；②每条关键声称附成本/延迟（L-COST）；③禁把「开源项目都这样做」当最优证据。
