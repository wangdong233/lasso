...
3. **CN/全球引擎混布 + 归因（中）**：US 原生搜索对 CN web 覆盖差（doc/21 实测双向验证码）；served_by/结构化结果/引擎选择是黑盒原生给不了的。残余价值诚实说：这是「策略层」价值，不是「质量」价值——Gemini 3 SimpleQA 72.1% SOTA 后「专用搜索质量更好」在英文顶端已不成立（S 级）。
...
- **证据纪律**：本轮拒采信项——zero-base §1.2 占比表（E 级，仅作排序不作数字）；scan-edge 的 star 数（二手，不影响任何裁决）；「Brave 免费额度需绑卡」沿用 doc/21 S 级但标注未复核绑卡环节。裁决官新增亲验 12 项（见文件头抽验记录）。
