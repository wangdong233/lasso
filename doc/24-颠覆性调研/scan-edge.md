...
> 方法论纪律（doc/23）：①零基视角（用户要什么/约束是什么），不从「与 Lasso 对比」出发；②每条发现附成本/延迟数据（L-COST）；③禁把「开源项目都这样做」当最优证据。
...
| 路径 | 延迟 | 金钱 | 备注 |
|---|---|---|---|
| Lasso serp_http 快探 | ~1s | $0（免费层/自托管） | doc/23 已确认的成本序基准 |
...
2. Lasso 若把 grep.app 私有 JSON 端点包进 search 链，会绑定无 SLA 的非官方接口——重演 doc/22 类型的运营风险；
