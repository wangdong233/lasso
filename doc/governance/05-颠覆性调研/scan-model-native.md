...
> 方法论纪律（doc/governance/04）：零基视角（用户要什么，不是「与 Lasso 对比」）；每条关键声称带来源；附成本/延迟数据（L-COST）；**禁止**把「开源项目都这样做」当最优证据。
...
| Brave（Lasso 链内） | $5 | 2026-02 起无免费档，$5/月赠送≈1k 次、需绑卡（R：doc/governance/02 verdict 官方页亲取） | 按次 |
| Kimi 原生 | ~¥30（≈$4） | 无 | 按次 |
...
**③ CN/全球引擎混布与内容覆盖（中强）**。US 原生搜索对 CN web 索引差（本仓库 doc/governance/02 verify 实测：零 key 英文路径 DDG/brave 双验证码；反向同样成立）；原生搜索不可指定引擎/区域/归因。Lasso 返回结构化结果 + `served_by` 归因（R：types.ts），策略可编排。

**④ 多源冗余与降级链（中）**。原生搜索是单厂商黑盒：厂商限流/故障时无处可退。Lasso 六层链（machine_mcp→zhipu→brave→serp_http→browse_headless→replay）+ 熔断器是架构性的可用性资产。但注意方法论纪律：**「多引擎冗余」的价值要用真实故障频率证明，不能用架构美感证明**——doc/governance/04 已教训过。
...
| R1 | Lasso 链条/成本/Brave 2026-02 免费档取消 $5/千次 | 本仓库 doc/governance/02-搜索方案重审/verdict + KEY-GUIDE（2026-08-17 核实）+ src/benchmark/run-ab-benchmark.ts |
