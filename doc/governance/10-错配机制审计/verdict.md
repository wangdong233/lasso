...
1. **注释漂移**：`test/unit/fetch-url.spec.ts:10` 头注释「ENOTFOUND → didnt」与 Y2 修复后行为矛盾 → 改「ENOTFOUND → unknown（v1.18.2 doc/governance/10 Y2）」。
...
3. **部署模型文档**（任务项 2）：`ARCHITECTURE.md` §1 新增「部署模型声明（守卫设计判据）」——单用户本地 stdio + 四条判据（多租户想象默认放开 / 瞬态≠策略拦截 / 瞬态不得升级惩罚 / 豁免区清单）；`doc/usage/01-KEY-GUIDE.md` §E 表新增 `LASSO_CALLER_CAP_DEFAULT` 行（默认不限制、opt-in 自控、计数保留观测）。
