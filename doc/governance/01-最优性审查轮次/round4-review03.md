...
- **文档面清点（1.4 项 7）**：README.md / doc/usage/01-KEY-GUIDE.md 对 `PerformanceObserver`、`network_timeout_ms` 零提及 → 显式标记 not-affected ✓。types.ts JSDoc 即 npm 消费者的 API 文档面（IDE hover），本轮修正使其不再随 v1.13.0 固化失实契约描述——这正是 T4-1 的收益实体。
