...
- **日期**：2026-08-19
- **裁决官**：修复与文档官（doc/governance/09 第三轮；输入 = audit.md §4 分级 + verify.md 真机矩阵 + 用户授权「GO 项实施；候选如登录完成自动 hide 若裁决 GO——注意防误判：检测登录墙消失 + 延迟 N 秒 + 仅对 lasso 台账 Chrome + 失败降级不 hide」）
...
| # | 项 | 理由 | verify 佐证 |
|---|---|---|---|
| N1 | desktop cgEvent「静默化」 | 物理边界（C 类）：CGEvent 合成的就是物理键鼠事件，静默即失能（KEY-GUIDE §C 诚实声明；doc/governance/08 fix.md 固化） | — |
