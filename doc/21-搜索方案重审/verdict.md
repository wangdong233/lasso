...
> 白盒抽验范围：src/search/FallbackChain.ts、src/tools/search.ts（schema/装配/fallback_chain 分支）、src/channels/BraveChannel.ts、src/config/providers.ts、src/config/quota-ledger.ts、src/serp/{extract,selectors,SerpHealthMonitor}.ts、src/doctor/doctor.ts（checkBraveKeys）、doc/KEY-GUIDE.md（d3d1b24 后）、test/ 相关 spec、check-invariants.mjs 相关 INV。
...
- **改法**：
  ...
  3. `src/doctor/doctor.ts` checkBraveKeys：:1106 next_step 删「免费 2000/月」改为「付费计划含 $5/月赠送额度（≈1000 次），需绑卡——免费路径用智谱+实搜兜底即可，详见 doc/KEY-GUIDE.md」；:1112 改为从 provider 配置读数（`N × free_quota_per_month`），消灭硬编码 2000。
