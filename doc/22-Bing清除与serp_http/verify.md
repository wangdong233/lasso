...
**结论**：serp_http 命中时 **1.9s vs 浏览器路径 5.3–5.9s（2.8–3.1× 提速）**，且在本机网络下快探层是**唯一能产出真实结果的零 Key 路径**（真 Chrome 反吃 brave 验证码 → count 0，与 doc/21 实测一致）。上表 browse 计时为 npx 缓存已预热口径（冷启动更慢）。
...
| 维度 | 结论 | 要点 |
|---|---|---|
| 1.1 代码规范 | ✅ | tsc 零错；error 桶命名一致（`serp_http_*`）；注释全部 WHY + 证据引注（doc/21、真机日期） |
