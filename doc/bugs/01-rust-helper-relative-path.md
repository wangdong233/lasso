...
>
> **仓迁注(2026-08-27)**:本文档写就时仓在 `/Users/wangdong/Documents/Project/cc-control-all/lasso`,现已迁至 `/Users/wangdong/Documents/Project/claude技能/lasso`——文中出现的旧绝对路径(含 §内 env 注入示例)均为当时历史现场,今已失效勿照抄;`~/.claude.json` 的 `LASSO_RUST_HELPER_PATH` 已由主循环重定位到新仓,且 v1.18.4 根治后该 env 覆盖本身已非必需(见 TROUBLESHOOTING §2.16 rustSpawnGate 四态)。**净版注(2026-08-27 补)**:代码修复 lineage 在 v1.18.4,但 31cd0cc 将 1.18.1~1.18.4 发布面弃用(受污区间系 dee853d 安全清理所致)——**用户可得的净版是 v1.18.5**。
