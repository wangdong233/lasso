...
2. **doc/KEY-GUIDE.md**：B 节边界表述按 verify.md §7 建议重写 + v1.17.2 精修 note。
3. **doc/TROUBLESHOOTING.md**：新增 §8.0 速查（指向 doc/27 + 用户向矩阵）+「第一个 tab 被导走」症状条目（版本判定 + 降级态日志锚 `logged_in_own_page_*`）。
