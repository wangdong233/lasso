...
- **实施范围**：仅 GO 两项（G1 测试+文档 / G2 描述运行手册），**零行为变化**（D-1b/D-2/D-3 均为 DECISION 项，未动）；版本不 bump（无行为变更，与 doc/27 fix.md 的 bump 惯例区分——本轮纯测试+文档+描述文案）。
...
- **doc/KEY-GUIDE.md §B**：同款三步 + `chrome-show` 可逆说明 + v1.8 note 同步修正（「登录必须走 visible 档」）。
