...
> Lasso（npm `lasso-mcp`）doc/ 目录索引。更新：2026-09-16（v1.27.0 预写态 · BUG-09 doc 收卷轮——补 bugs/09、新鲜度对账至 BUG-09 三轮对抗复审后；上轮 DocAudit 见 git `9ed9cfc`，删除类处置见 [整理决议-2026-09-16.md](整理决议-2026-09-16.md) 先报后删）。
...
| [`bugs/09-2026-09-16-反爬根治通道与url语义统一决议.md`](bugs/09-2026-09-16-反爬根治通道与url语义统一决议.md) | 反爬根治通道（防御梯 L0/L1/L2 + `browse_headed` 有头档两态生命周期）+ url 语义统一（单一 ensure-navigation，假数据面根治）+ P2 批 C1-C5；r1 设计修订 + §5r2/§5r3 两轮对抗否定复审 + §7 实施定稿 + §8 尾款轮 | **定稿（部分实施，v1.27.0 预写）**：B/C1-C5/HeadedChannel 已合并三轮复审 clean；**WT4 驱逐哨兵已由尾款轮 §8.A 清偿**（A.5r2 三信号漏斗 + INV-98 真锚）；开放项见 §5/§7.2（开放项 6、R-INT-06 触发登记、分支 ref 卫生） |
...
| ../ARCHITECTURE.md | v1.27.0 预写态（BUG-09 批增补：工具面 18 / HeadedChannel 通道树 / §11 INV 编号 99 实装 98 / §16.7 / §14-§15；计数为收卷轮实跑 3090+1 / 190 文件） | 2026-09-16 |
...
| testing/01（功能测试清单） | v1.27.0 预写（BUG-09 收卷轮补 R-INT-06 后记④触发登记——url 语义统一批触发「抽导航语义模块」义务未执行，顺延） | 2026-09-16 |
...
| bugs/03-09 | v1.21.0 / v1.22.0+附录E / v1.23.0 / v1.24.0 / v1.25.0 / v1.26.0 已发布 / v1.27.0 预写（部分实施） | 2026-09-07~16 |
| 根级渲染档文档组（7 件） | v1.19-v1.20 落地面 + 登记开项 | 2026-09-10（登记 #10 最新核） |

## 历史遗漏盘点（BUG-09 doc 收卷轮，2026-09-16）

上轮 DocAudit（`9ed9cfc`，446→17 整理）之后 doc/ 新增文件仅一件：`bugs/09-2026-09-16-反爬根治通道与url语义统一决议.md`（本批设计决议 + 三轮复审 + §7 实施定稿，已入 ⑤ 表）——**无遗漏未归位文件**。`整理决议-2026-09-16.md`（DocAudit 轮删除清单）按其「先报后删」性质保留在根级作审计记录，结构树已列。src/test 侧本批新增 spec（headed/bug09-r2/r3 族）属代码树测试载体，不属 doc/ 归档面。
