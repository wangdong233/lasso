...
> Lasso（npm `lasso-mcp`）doc/ 目录索引。更新：2026-09-18（bug11 批**收卷终态**——U-D `351ac29` + U-T `fd7d800` 双单元全合并 + 对抗否定复审轮 1 clean〔G2 追加语义 P1 修复 + README 双语重复条目收敛〕；实机报告〔下午〕处置后记入档 + bugs/11 状态转「已实施收口」+ 计数 3239+4/199/103；README v1.29.0 合并线双条目收敛为单条〔中英双语〕）；前态 2026-09-17（v1.28.0 预写态 · BUG-10 doc 收卷终态——实机报告处置后记 + bugs/10 状态转「已实施收口」+ ARCHITECTURE §16.8 回执体系/计数 3186/196/103 + 排期表 v1.27.0/v1.27.1 转已发布 + v1.28.0 行 + testing/01 ④″ 触发登记 + README v1.28.0 前缀统一〔中英双语〕；bug11 批增量——governance/13 合成输入伦理边界声明新增 + usage/02 §2.19/§2.20；上轮 doc 收卷见 git `d635576`/`9b7ee97`，删除类处置见 [整理决议-2026-09-16.md](整理决议-2026-09-16.md) 先报后删，bugs/11 索引行 + 排期映射扩至 03-11）。
...
├── testing/      功能测试清单                          01
├── governance/   审计与裁决档案（轻量 ADR，按时间线）   01-13
...
| 2026-09-17 | [`governance/13-合成输入伦理边界声明.md`](governance/13-合成输入伦理边界声明.md)（1 件） | 输入原语面伦理契约（bugs/11 决议 A.2 持久化外置）：正面面 = type/press 标准原语 + input_guard 诚实信号 + 信任阶梯；拒绝面 = 节奏拟态/tracker 欺骗/accessor 反劫持/isTrusted 伪造；立场 = 墙在那里我们告诉你，穿墙两形态（换可信原语 / 请用户物理输入）交调用方；三问判定准绳 + 用户显式裁决推翻程序 | 定稿（bug11 批；长期契约——后续输入能力演进先对照 §2/§3） |
...
| [`bugs/10-2026-09-17-商标站滑块批-cgEvent投递证伪与回执语义决议.md`](bugs/10-2026-09-17-商标站滑块批-cgEvent投递证伪与回执语义决议.md) | 商标站滑块批：cgEvent 投递断裂**证伪**（三路白盒 + 受控计数器页——真凶=并发物理输入竞争）→「修复」重定向为**消灭静默失败**（决议 A：landing 回执 / 物理竞争检测 / doctor 光标 wiggle 自检〔opt-in〕/ TCC 探测诚实化）+ 决议 B evaluate 表达式自动返回（P2 静默错值根治）+ 决议 C desktop 截图默认落盘（P3 token 爆炸）+ 决议 D 检测面文档（P1 拦截面位移——TROUBLESHOOTING §2.18）；§5 双 worktree 拆分（U-R rust+desktop / U-B browse+docs，文件域互斥）；§9 对抗否定复审 + 终局复验 **adversarial: clean**（五路变异全捕获——2 存活当场修为 INV-101(2b)(2c) 变异锚；gate 4/4） | 定稿（2026-09-17，**已实施收口**——U-R 合并 `71434b5` / U-B 合并 `3c0af58` / 复审修正 `fa7a56a`；实机报告附处置后记表） |
| [`bugs/11-2026-09-17-输入保护层证伪-type原语补齐与诚实信号决议.md`](bugs/11-2026-09-17-输入保护层证伪-type原语补齐与诚实信号决议.md) | 输入保护层「瞬间清除」**证伪**（真机理=React 16/17 受控回写 + blur 回写 + accessor 劫持——脚本设值不进框架管道，可信键入存活）→ 决议 A 边界裁定（原语补齐+诚实信号 vs 主动对抗 REJECTED-BY-DESIGN）+ 决议 B `type`/`press` 两 action（上游 type_text/press_key 映射，禁 CDP 直连）+ 决议 C `input_guard_suspected` 三探针信号（纯读零副作用）+ 决议 D P2 观察面/P3 no_reload 语义钉 + 决议 E 双 worktree（U-D/U-T 文件域互斥）；§5.1 预算工序（D-θ：fleet/L2 余量实测 + 等量裁剪/提帽纪律） | 定稿 r2（2026-09-17，**已实施收口**——U-D 合并 `351ac29` / U-T 合并 `fd7d800`〔D-γ 契约表 + HighRiskGate D-ζ 补拦 + 提帽 `f31f3d4`/`4c05f64`/`f4d0166`〕+ 对抗否定复审轮 1 **adversarial: clean**〔G2 追加语义误报 P1 修复 + README 双语 v1.29.0 合并线重复条目收敛；四路变异全捕获；gate 4/4：199 files / 3239+4 skipped / INV 103/103〕——v1.29.0 预写） |
...
| 文档 | 写到版本 | 最后同步 |
|---|---|---|
| ../README.md | v1.28.0 预写（BUG-10 **双半齐**——收卷轮统一前缀为「未发布预写 · 上半/下半」并修 v1.27.0 中文条目重号残留；版本行仍 v1.27.1 随 package.json，发版时再升） | 2026-09-17 |
| ../ARCHITECTURE.md | v1.28.0 预写态（BUG-10 收卷轮增补：§11 INV 103 / §14 v1.27.0 转已发布+v1.27.1+v1.28.0 要点 / §10-门禁 计数 **3186 passed + 1 skipped / 196 文件 / 103 INV 实装** / §15 主线 11 / **新 §16.8 桌面合成输入回执体系**） | 2026-09-17 |
| usage/01（KEY-GUIDE） | v1.28.0 预写（BUG-10 U-R 批补 `LASSO_DOCTOR_INPUT_SELFTEST`；此前 BUG-09 W2 批 `LASSO_HEADED_IDLE_MS` / `LASSO_HEADED_HARD_CAP_MS` / `LASSO_FALLBACK_CROSS_CHANNEL` + file-from 三工具） | 2026-09-17 |
| usage/02（TROUBLESHOOTING） | v1.29.0 预写（bug11 决议 D：新增 §2.19 window.open 新 tab 观察面 + §2.20 no_reload 语义矩阵）；v1.28.0 预写（BUG-10 决议 D：新增 §2.18 tm.aliyun.com 分层拦截面实测 + 「click worked ≠ 提交被接受」通则；v1.27.0 批 §2.17 驱逐释义；v1.26.0 批 §10 四配方） | 2026-09-17 |
...
| architecture/02（排期） | v1.28.0 预写（BUG-10 收卷轮：v1.27.0 / v1.27.1 行转 ✅ 已发布 + 新增 v1.28.0 ⏳ 行） | 2026-09-17 |
| testing/01（功能测试清单） | v1.28.0 预写（BUG-10 收卷轮补 ④″ **三次触发登记**——U-B 触碰 BrowseChannel.ts 2679→2915 行，模块抽出仍未执行，触发计数 3 次未执行 + escalation 提案；此前 BUG-09 轮 ④/④′ 登记） | 2026-09-17 |
...
| governance/13 | v1.29.0 预写（bug11 决议 A.2 持久化外置——长期契约） | 2026-09-17 |
| bugs/01-02 | v1.18.4 / v1.18.6（已根治） | 2026-08-23/31 |
| bugs/03-11 | v1.21.0 / v1.22.0+附录E / v1.23.0 / v1.24.0 / v1.25.0 / v1.26.0 / **v1.27.0 / v1.27.1 已发布** / v1.28.0 预写（bugs/10，**收卷终态**——已实施收口 + 对抗复审 clean） / v1.29.0 预写（bugs/11，**收卷终态**——U-D+U-T 双单元已实施收口 + 对抗复审 clean） | 2026-09-07~18 |
...
## 历史遗漏盘点（BUG-10 doc 收卷轮，2026-09-17）

- **新增文件对账**：上轮（`9b7ee97`）之后 doc/ 新增 = bugs/10 决议 + 实机报告 + 证据 6 png（`276b68e` 一次性入树）+ usage/04 工具示例体系（`0bec23e`，v1.27.1 批——上轮盘点文成于其落档前，索引行已在位，非遗漏）；全部归位，`git status --porcelain` 零输出（本轮收卷前核）。
- **状态漂移修正（本轮全部处置）**：① bugs/10 头部「定稿（待实施）」→「已实施收口」（实施证据锚：`71434b5`/`3c0af58`/`fa7a56a`）；② 实机报告补处置后记表（含 P0「断裂」判定被证伪的显式更正——标题不回改，勘误上版）；③ 本索引 3 处「实施中」措辞收敛；④ 排期表 v1.27.0 行「⏳未发版」过期（实际 v1.27.0 `0cbd950` / v1.27.1 `81e882d` 均已 09-16 发布）→ 转已发布 + 补 v1.27.1 行；⑤ ARCHITECTURE §14「当前版本 npm latest 1.26.0」过期两版 → 1.27.1。
- **冗余修正**：README 中文 changelog v1.27.0 条目重号残留（`- **v1.27.0**：- **v1.27.0**：`，合并期产物，英文版无此病）已修；v1.28.0 双条目前缀不一致（中文上半「开发中」/下半「未发布预写」；英文同病）统一为「未发布预写 · 上半/下半」（readme-sync D 校验现可实际覆盖 v1.28.0——此前括号内前缀形态逃逸版本集合正则）。
- **义务登记**：testing/01 补 ④″ 三次触发登记（BUG-10 U-B 触碰 BrowseChannel.ts，模块抽出未执行，触发计数 3——含 escalation 提案：连续三批触发未执行，下批 review 应列硬项，归用户裁决）。
- **观察项（不改，记录在案）**：① `整理决议-2026-09-16.md` 仍标注「临时文件」——其「先报后删」的删除动作已执行完毕（上轮），文件本体现为删除决策的审计残页；保留（删除审计史需可追）或归档 `archive/` 均可，归用户裁决，本轮不动。② ARCHITECTURE §16.7 标题「v1.27.0 预写态」转正为已发布（本轮已改）；§16 各小节标题的「预写态」后缀在发版后由收卷轮统一转正——本条即先例。③ rust `#[test]`+`#[tokio::test]` 计 178（2026-09-17 grep 实测）与 ARCHITECTURE 旧文案「cargo test（207）」口径不符——旧数无法复算（疑含非 `#[test]` 形态计数），已改为可机械复算的 grep 口径并入档。

## bug11 批 doc 增量登记（2026-09-17，v1.29.0 预写）

- **新增文件对账**：本批新增 = `governance/13-合成输入伦理边界声明.md`（bugs/11 决议 A.2 持久化外置——长期契约非快照）。usage/02 §2.19（window.open 新 tab 观察面）/§2.20（no_reload 语义矩阵）为既有文件内增长。`bugs/11` 决议文件与实机报告由主线收口 commit 入树（**已入树**——2026-09-18 收卷 commit；实机报告附处置后记 + 四图证据同批）。
- **索引行**：governance 表 + 排期表各 +1 行（见上）；usage/02 排期行补 v1.29.0 预写前缀。
