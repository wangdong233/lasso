...
> Lasso（npm `lasso-mcp`）doc/ 目录索引。更新：2026-09-17（v1.28.0 预写态 · BUG-10 **doc 收卷终态**——实机报告处置后记 + bugs/10 状态转「已实施收口」+ ARCHITECTURE §16.8 回执体系/计数 3186/196/103 + 排期表 v1.27.0/v1.27.1 转已发布 + v1.28.0 行 + testing/01 ④″ 触发登记 + README v1.28.0 前缀统一〔中英双语〕；**bug11 批增量**——governance/13 合成输入伦理边界声明新增 + usage/02 §2.19/§2.20；上轮 doc 收卷见 git `d635576`/`9b7ee97`，删除类处置见 [整理决议-2026-09-16.md](整理决议-2026-09-16.md) 先报后删；本轮（v1.29.0 预写 · bugs/11 U-T 半）：bugs/11 索引行 + 排期映射扩至 03-11）。
...
| bugs/03-11 | v1.21.0 / v1.22.0+附录E / v1.23.0 / v1.24.0 / v1.25.0 / v1.26.0 / **v1.27.0 / v1.27.1 已发布** / v1.28.0 预写（bugs/10，**收卷终态**——已实施收口 + 对抗复审 clean） / v1.29.0 预写（bugs/11 U-T 半——type/press 原语 + 诚实信号） | 2026-09-07~17 |
...
- **新增文件对账**：本批新增 = `governance/13-合成输入伦理边界声明.md`（bugs/11 决议 A.2 持久化外置——长期契约非快照）。usage/02 §2.19（window.open 新 tab 观察面）/§2.20（no_reload 语义矩阵）为既有文件内增长。`bugs/11` 决议文件与实机报告由主线收口 commit 入树（本登记成文时未在库——链接在其入树后生效）。
