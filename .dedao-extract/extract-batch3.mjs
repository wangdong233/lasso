...
// ---------------- lasso MCP 客户端 ----------------
const client = new Client({ name: "dedao-extract-b3", version: "1.0.0" });
const transport = new StdioClientTransport({
  command: "/bin/zsh",
  args: [
    "-c",
    `exec node dist/index.js 2>>${JSON.stringify(path.join(OUT, "server-b3-stderr.log"))}`,
  ],
  cwd: ROOT,
...
const JS_CLEANUP = `return (()=>{
  ...
  // v3：高度归一化——SPA 显式 height/min-height 残留会在删除后留下整页空白尾页（发刊词 p15 实证）
  ...
  // v3.1：尾部 margin 归零——rect 诊断看不到 margin，.article 内最后可见后代链的
  // margin-bottom 会把布局高度顶过页边界 → 整页空白尾页（发刊词 +20px 实证）
  const chainEls=[wrap];
  let node=wrap;
...
save("b3-98-results.json", results);
save("b3-99-log.json", log);
note("\n===== SUMMARY =====");
